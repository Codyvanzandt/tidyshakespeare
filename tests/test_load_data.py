...
def test_load_single():
    expected = pandas.read_csv("data/csv/hamlet.csv", delimiter="\t", quotechar="|", index_col=None, header=0)
    assert load("hamlet").equals(expected)

def test_load_multiple():
    hamlet = pandas.read_csv("data/csv/hamlet.csv", delimiter="\t", quotechar="|", index_col=None, header=0)
    dream = pandas.read_csv("data/csv/a-midsummer-nights-dream.csv", delimiter="\t", quotechar="|", index_col=None, header=0)
...
def test_load_all():
    ...
    dataframes = [
        pandas.read_csv(f"data/csv/{play_name}.csv", delimiter="\t", quotechar="|", index_col=None, header=0)
